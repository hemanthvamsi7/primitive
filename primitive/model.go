...
func (model *Model) Step(shapeType ShapeType, alpha, repeat int, minQuadWidth float64, maxQuadWidth float64) int {
	//state := model.runWorkers(shapeType, alpha, 1000, 100, 16)
	state := model.runWorkers(shapeType, alpha, 500, 50, 8, minQuadWidth, maxQuadWidth)
...
func (model *Model) runWorkers(t ShapeType, alphaVal, n, age, m int, minQuadWidth float64, maxQuadWidth float64) *State {
	...
	for i := 0; i < wn; i++ {
		...
		go model.runWorker(worker, t, alphaVal, n, age, wm, ch, minQuadWidth, maxQuadWidth)
	}
...
func (model *Model) runWorker(worker *Worker, t ShapeType, a, n, age, m int, ch chan *State, minQuadWidth float64, maxQuadWidth float64) {
	ch <- worker.BestHillClimbState(t, a, n, age, m, minQuadWidth, maxQuadWidth)
}
